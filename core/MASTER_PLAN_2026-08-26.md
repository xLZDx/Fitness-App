...
| 15 | `kCanonicalMachines` second source of truth (`gemini_equipment_service.dart:143`) still drifts from `equipment.json` | Medium *(new — architect)* | Open; the 08-03 fix closed one instance, not the mechanism | Closes via OBS-1 item 10 | §2, §8 |
| 16 | Gemini/Firebase AI Logic client-direct calls — no rate-limit/abuse-cost monitoring | **Medium-High** *(new — security review)* | Open, real exposure if App Check (item 14) is incomplete on this path | Closes via OBS-1 item 9 | §6, §8 |
| 17 | No client-runtime/ML telemetry (frame-timing, camera/inference failure rate); no `firebase_performance` present | Medium *(new — flutter-reviewer)* | Open — Crashlytics exists, performance/jank monitoring does not | Closes via OBS-1 item 11 | §8 |
| 18 | Mobile/Dart dependency (SCA) scanning absent (backend already covered via `npm audit` in CI) | Medium *(new — security review)* | Open, asymmetric coverage | Closes via OBS-1 item 6 | §8 |
...
- **B1 (BLOCKER, FACT):** `targetSdk`/API-36 deadline (5 days out) was ranked below OBS-1 despite being the only hard external, non-negotiable cutoff in the table. **Applied** — §9 row 1.
