...
- **P2** (OCR text-recognition engine) — *updated 2026-09-16.* **P2.G1** (structured OCR capability),
  **P2.G2** (IdentityTextParser), and **P2.G3** (server exact text lookup,
  `functions-equipment-identity/`) all **CLOSED**. P2.G3's implementation was reviewed and pushed
  2026-09-11 (5 GPT-PM rounds, `VERDICT: APPROVE`, 0 open BLOCKER/MAJOR); the real, non-dry-run
  production deployment that gate was waiting on happened 2026-09-16, under a fresh, specific
  GPT-PM APPROVE naming the deploy action itself (2 rounds — round 1 MAJOR on unverified live
  Firestore-index inventory, closed with real evidence; round 2 APPROVE). `firebase deploy --only
  functions:equipment-identity,firestore:indexes --project fitness-app-korostelev` — "Deploy
  complete!", two functions live (`equipmentIdentityResolveFromText`, `equipmentIdentityRecordTelemetry`,
  both europe-west1), one Firestore composite index live. Verified post-deploy, live: 22 Cloud
  Functions now deployed (was 20), and `stripeWebhook`'s source generation is byte-identical to its
  pre-deploy value — the production Stripe path was provably untouched. Full evidence in
  `core/DECISION_LOG.md`, "Tier D item 15... DONE." P2.G4+ not started.
...
| 12 | Nonprofit/subscription ARB copy contradiction | Medium | **Closed 2026-09-16** — GPT-PM VERDICT: APPROVE (option A: remove donation-solicitation framing, keep the mission/safety-paywall-free claims, describe subscriptions plainly). Only `momentsWeReANonprofitAndThe`'s value actually carried donation-register language ("a few coffees of your time, would you consider..."); the Terms/Privacy text (`legalTermsBody`/`legalPrivacyBody`) already correctly disclaimed nonprofit status. That one EN+RU value rewritten; nothing else needed changing | Closed, `core/DECISION_LOG.md` "Tier C rows 12/19" | §6 |
...
| 19 | R11f-2 — photo export policy decision | Low | **Closed 2026-09-16 — WONT_BUILD** — GPT-PM VERDICT: APPROVE. Progress-photo export stays unbuilt by default to preserve the "never leaves your phone" privacy promise; the old prototype `ExportScreen` (`App.tsx:3931`) is formally superseded, not a pending decision. Reopenable only as a fresh, explicitly consent-gated, single-photo export gate if real user demand ever justifies it — not a resumption of the old design as-is | Closed, `core/plans/PLAN_BUGS_2026-08-13.md` R11f-2, `core/DECISION_LOG.md` "Tier C rows 12/19" | §4 |
...
| 24 | Firestore rules grant read/write over `/users/{uid}/{coll}/**` via a wildcard match with a hand-maintained per-write denylist, rather than an explicit per-collection allowlist | Medium *(new — 2026-09-15, §12)* | **Closed 2026-09-16 (P2-ACCESS-1)** — GPT-PM APPROVE (4 plan revisions, then a 4-round implementation review, terminal round explicitly authorized as an exception to this project's 3-round cap, itself GPT-PM-approved). New `scripts/ci/check_data_access_policy.js` CI gate: 4-source discovery (rules full-path, literal `.collection()`/`.doc()` across all 3 production codebases, a registered path-registry, fail-closed on everything else unresolved) requires a path-aware `clientAccess` declaration for every discovered collection, verified against the real Firestore emulator with operation-specific CONDITIONAL proof. Backfilled for all 37 discovered paths. 30-class mutation-proof self-test. The wildcard/denylist PATTERN itself in `firestore.rules` is untouched (full allowlist migration remains a deferred option); this closes the OMISSION risk — a new sensitive collection added without a deliberate access declaration now fails CI, closing exactly the P2.G5 `equipment_identity_telemetry` scenario this row named | Commit `6c8d8bb`, full 21-defect review history in `core/DECISION_LOG.md` (search "Row 24 gate") | §12 |
...
| 26 | Legacy health-data lazy migration leaves the server-side health block indefinitely for an account that never reopens the app; the purpose-built one-off remediation script (`scripts/ops/strip_health_from_profiles.py`) exists but is run by hand, on no schedule, with no on-record dated zero-result evidence of a completed sweep | Medium *(re-flagged — this exact gap was already required to be closed or softened by `core/audit/gate_j_regulatory_review_2026-08-15/GATE_J_REGULATORY_REVIEW_2026-08-15.md:66`; not new, but still open as of 2026-09-15, §12)* | **One-off sweep completed 2026-09-16, operator-authorized ("ГО А")** — `strip_health_from_profiles.py --apply` run against both `default` (39/39 cleared) and `legacy-shared` (1/1 cleared), 0 failed; a fresh dry run against both afterward confirmed 0 residual stale documents. "We do not store health data" is now true for every currently-known stored profile. The underlying gap (no schedule, run by hand) is **not** closed by this — a real, testable, scheduled mechanism (same treatment P2.G5-readiness's own step 4 is building for a different collection) is still open work | `core/DECISION_LOG.md`, "Legacy health-data residue swept" | §12 |
