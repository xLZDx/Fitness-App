...
`submission.json` as issued, with the four fields you fill left empty:
...
`reviewed_at` is ISO-8601 **with an offset** (`2026-08-20T14:00:00+01:00`); a timestamp with no
zone cannot be ordered against a catalogue change. `rows_csv` points at the filled spreadsheet
beside this file and is expanded into the `rows` the validator reads — send the two together, and
do not move the CSV elsewhere, because a review tied to a sheet nobody sent back cannot be tied to
a catalogue version either. The three pre-filled fields pin the review to exact bytes; editing them
does not make a review of different bytes valid, it only makes the mismatch harder to see.
