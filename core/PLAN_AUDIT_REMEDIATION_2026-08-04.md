...
## Гейт P0 — настоящие Условия и Политика конфиденциальности (2026-08-05)

**Почему гейт возник.** Проверка готовности к Play упёрлась в
`privacy_page.dart:21`: `kIsPlaceholderContent = true`, а тело страницы прямо
говорило «Replace it with the operator's reviewed Privacy Policy **before this
app is submitted to any app store**». Google требует публичный URL политики, а
приложение собирает данные о здоровье — категория, которую проверяют вручную.

**Что сделано.** Канонический текст обоих документов (EN + RU) живёт в
`scripts/legal/legal_text.py`; `scripts/legal/build_legal.py` генерирует из
него и ключи `.arb` для экранов в приложении, и `public/{privacy,terms}.html`
для Hosting. Один источник, два потребителя — три рукописные копии
юридического документа расходятся, а разошедшийся документ перестаёт быть
тем, с чем пользователь соглашался.

**Каждое утверждение проверено по коду**, а не взято из шаблона. Полное
соответствие claim -> file:line в шапке `legal_text.py`. Ключевое:

- данные о здоровье не уходят в ИИ (`ai_coach_service.dart:33-40` — весь
  промпт это название тренажёра и язык);
- фотография тренажёра **уходит** в Gemini (`gemini_equipment_service.dart:37-48`);
  это раскрыто явно, а не спрятано под «мы можем использовать сторонние сервисы»;
- фото прогресса не покидают телефон (`progress_photos_providers.dart:13`);
- рекламы и аналитики нет вовсе (`pubspec.yaml`);
- удаление необратимо (`index.ts:1169-1207`);
- цены сверены с `subscription_page.dart:960-977`, комиссия тренера 15% — с
  `index.ts:985`.

**Флаг `kIsPlaceholderContent` удалён, а не переключён в `false`.** Булев
флаг, который никто не читает, — это ровно та несвежая переменная, о которой
предупреждал его собственный комментарий. Вместо него
`legal_pages_test.dart` проверяет сам отрендеренный текст, включая два
утверждения, снятых в S0b: «has not been reviewed by a physiotherapist» и
«not tax-deductible donations». Регресс, который вернёт маркетинговую
формулировку, теперь падает тестом, а не доезжает до магазина.

**Проверки.** `flutter test` — 1275 passed. `flutter analyze` — 6 замечаний,
все существовали до гейта. Живьём: `/privacy` и `/terms` отдают HTTP 200,
оба языка на месте, вхождений «placeholder» — 0.

**Ответы для формы Data safety** выведены из кода и лежат в
`core/PLAY_DATA_SAFETY_2026-08-05.md`.

### Что осталось непокрытым

1. **Бэкенд S0b не был вычищен.** `generateAnnualReceipt` до сих пор
   возвращает `orgName: "Fitness App (501(c)(3) pending)"` (`index.ts:826`,
   `:878`) и notice о ретроактивной налоговой вычитаемости под фискальным
   спонсором (`:880-884`), и записывает это в Firestore (`:889`). Тесты
   закрепляют ложную строку (`index.test.ts:407`, `:501`). S0b чистил только
   Flutter-слой. Клиентских вызовов нет, но эндпойнт задеплоен и вызываем, и
   он **прямо противоречит** только что опубликованным Условиям. Требует
   отдельного GO.
2. **Хранение данных в Firebase AI Logic** — от него зависит галочка
   «processed ephemerally» в Data safety.
