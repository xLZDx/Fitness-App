...
| **P5** | ~~`equipment_v1.tflite` not trained and not bundled — recognition runs cloud-only~~ **STALE (F012, corrected 2026-08-17).** The file is bundled and loaded: `mobile/assets/models/equipment_v1.tflite`, 4,496,661 bytes, trained 2026-07-29, declared in `pubspec.yaml`, copied out by `core/assets/asset_bootstrap.dart` and read by `MlKitVisualEquipmentService` and `MlKitLiveEquipmentService`. Recognition is cloud-FIRST with this as the on-device fallback, not cloud-only. What is true, and is the part worth keeping, is its **coverage**: a 10-way head over a 69-machine catalogue, measured top-1 0.617 / top-3 0.835 on a held-out 15% (n=261) — see `mobile/assets/models/README.md`, and `core/business/PITCH_2026.md` which F002 corrected to match |
