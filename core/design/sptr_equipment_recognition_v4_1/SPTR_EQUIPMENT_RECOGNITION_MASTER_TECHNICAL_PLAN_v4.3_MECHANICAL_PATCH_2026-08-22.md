...
# Equipment Recognition Master Technical Plan v4.3 — Mechanical Patch Candidate

Single design authority: v4.2 REMEDIATED RC + GPT-PM round-2 verdict (2026-08-22, PM Bridge, conversation `https://chatgpt.com/c/6a821d86-5720-83eb-b20b-a33f5b3cb5dc`) that this revision closes
...
| Document status | MECHANICAL PATCH CANDIDATE — closes GPT-PM round-2's 7 named invariants. Per GPT-PM's own round-2 verdict, round-cap is exhausted and no third full adversarial round is requested; this revision is pending **binary verification only** (do the 7 invariants literally appear, unambiguously, in binding text) before status may move to `v4.3 CONSENSUS`. |
| Predecessor status | v4.2 REMEDIATED RC — round-2 verdict: **NOT APPROVE**. All 4 original BLOCKERs (B-01..B-04) confirmed CLOSED. 1 new BLOCKER (P6-T/P6-V evidence-lane separation not machine-enforced) + 2 CRITICAL (ABSTAIN/infra conflation persists via `abstainReason`; authority tuple incomplete/not proven immutable) + 4 MAJOR (contract negotiation one-directional; `NEED_MORE_VIEW` wrongly excluded from P2.G5 denominator; App Check mode transition doesn't invalidate prior shadow evidence; push/listener revocation treated as sole correctness boundary) found on the **full document text**, not the changelog — full record below and in `core/review/` (verdict text logged verbatim via PM Bridge `gpt_ingest_reply`, 2026-08-22). |
| Production exact-model claim | Still BLOCKED until P6 sealed blind evaluation + statistical confidence bounds + calibration/open-set gate + model-by-model promotion + B-01..B-04 + this revision's 7 invariants all close |
...
```text
FINAL STATUS FOR v4.3

DESIGN STATUS: MECHANICAL PATCH CANDIDATE -- closes GPT-PM round-2's 7 named
  invariants (1 BLOCKER, 2 CRITICAL, 4 MAJOR) on top of v4.2's already-confirmed
  closure of B-01..B-04.
...
Per GPT-PM's own round-2 verdict, this document does not require a third open-ended
adversarial round. It requires ONE binary verification pass: confirm each of the 7
invariants below appears in this document's binding text with no formal bypass path,
then this document's status may move to v4.3 CONSENSUS without a further contested
review round.

  [ ] 1. server-derived TEXT_ONLY | VISUAL evidence lane           -- 6.4.1, 6.5, P6.G0
  [ ] 2. infra failures impossible under ABSTAIN                    -- 6.5 (abstainReason/
                                                                        failureCode split)
  [ ] 3. complete immutable RecognitionAuthorityTuple                -- 4.2, 4.6
  [ ] 4. actual client/server contract negotiation                  -- 6.5, P2.G3
  [ ] 5. NEED_MORE_VIEW remains in P2.G5 denominator                 -- 7.1, P2.G4, P2.G5
  [ ] 6. App Check mode change invalidates relevant shadow evidence  -- P0.G0, P6.G1
  [ ] 7. revocation correctness does not depend solely on
...
| **v4.3 MECHANICAL PATCH** | **2026-08-22** | **MECHANICAL PATCH CANDIDATE — pending binary verification** | Closes all 7 of GPT-PM round-2's named invariants: server-derived `evidenceLane` (new P6.G0 gate + §6.4.1), `abstainReason`/`failureCode` split, unified immutable `RecognitionAuthorityTuple`, request-side contract negotiation, `NEED_MORE_VIEW` restored to the P2.G5 denominator, App-Check-mode-transition shadow-evidence invalidation, and a revocation-correctness bound that no longer rests solely on push/listener delivery. Not yet CONSENSUS — awaiting the binary verification pass GPT-PM itself specified as sufficient. |
