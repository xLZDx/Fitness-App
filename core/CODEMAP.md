...
| `equipment` | 23 | 5,586 | Equipment detail + workout player. `data/exercise_filter.dart` holds the injury filter, and it **works**: `safetyCoverage()` in the same file measures 1,527 of 1,887 exercises carrying a contraindication tag, held there by the `kSafetyCoverageFloor` ratchet in `test/features/equipment/safety_coverage_test.dart`. This entry said "gates **nothing** … 0 of 1,887" until 2026-08-15; that was true when written and had been false for a long time. `widgets/exercise_thumb.dart` is the one tile every list renders an exercise as (bundled poster, gradient fallback) |
...
| `onboarding` | 15 | 2,043 | Multi-step intake incl. the injury questionnaire. `lib/features/onboarding/steps/` = one file per step; `widgets/ob_shell.dart` = the redesign chrome (chevron, progress track, step counter). **`data/step_answered.dart` is the flow**: `kOnboardingOrder` is the screen order, and the same file answers "has this section been touched" (the Skip/Next label) and "where does a returning user land" |
| `profile` | 11 | 2,212 | Profile + settings. `lib/features/profile/data/firestore_profile_repository.dart` is the Firestore boundary |
| `form_check` | 23 | 6,821 | On-device pose/form checking. `data/coach_phases.dart` owns `countingIsLiveIn` — the one answer to "may reps be counted right now", read by the guard in `RepSessionController._onFrame`. `data/mlkit_pose_detector_service.dart` + `data/form_classifier.dart`; `data/pose_silhouette.dart` builds the two-sided outline (drawn only — scoring still reads the six authored side-view joints) and `data/pose_projection.dart` maps measured landmarks onto the preview. `widgets/camera_flip_button.dart` is shared with `posture` — both open the front lens and both need the back one at a mirror for a full-body frame |
