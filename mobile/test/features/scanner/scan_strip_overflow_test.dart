...
/// The scan preview's top strip, rendered in the language the app ships in.
///
/// ## Why this test is in Russian
///
/// The strip overflowed by 12 pixels on a real device and no host test saw it,
/// because `test/helpers/test_app.dart` pins `en` for every widget test — a
/// deliberate choice that keeps assertions readable and stops a Russian copy
/// tweak from breaking a test that is not about translation. It also means the
/// suite measures every layout against the SHORTEST strings the app has.
///
/// "Scan" and "Live" are four characters each. Production pins `ru`
/// (`lib/main.dart`), where they are "Распознавание" and "Живой режим". So the
/// overflow was visible to every actual user and to none of the 2,039 host
/// tests, and it took the device walk to find it.
///
/// This one is pinned in `ru` on purpose. It is not a translation test — it is
/// a layout test, and Russian is simply the widest real input the strip gets.
    ...
    child: MaterialApp(
      ...
      home: MediaQuery(
        ...
        child: Scaffold(
          body: SizedBox(
            width: size.width,
            child: ScanTopBar(live: false, onLive: (_) {}),
          ),
        ),
      ),
    ),
...
void main() {
  ...
  for (final width in const [320.0, 360.0, 411.0]) {
    testWidgets('the scan strip fits at ${width.toInt()}px in Russian',
        (tester) async {
