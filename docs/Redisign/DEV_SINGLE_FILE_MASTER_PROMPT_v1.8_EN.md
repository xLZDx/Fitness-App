# FITNESS APP — SINGLE-FILE DEVELOPMENT MASTER PROMPT v1.8 (ENGLISH)
...
# FIGMA MAKE HANDOFF INGESTION PROTOCOL — v1.8
...
# REMAINING IMPLEMENTATION SCOPE — EMBEDDED v1.8
...
Verify and preserve existing capabilities, then align their UI with Figma Make:

- scan history;
- saved machine cards and My Machines;
- camera permission flow;
- analysing, result, and retry;
- unknown-equipment flow;
- offline or on-device fallback;
- network failure;
- low-confidence result;
- manual search fallback only if approved and supported;
- Scan-to-Equipment navigation;
- contextual AI Coach entry;
- no silent conversion of an unknown machine into the nearest incorrect result.

If cloud recognition and local fallback already exist, do not create another pipeline and do not change ML architecture without separate GO.
...
# FIGMA MAKE → GITHUB REFERENCE REPOSITORY PROTOCOL — v1.8
