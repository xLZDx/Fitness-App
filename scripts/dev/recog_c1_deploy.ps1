...
if ($Push) {
    ...
    Write-Host ""
    Write-Host "The SIXTH one nothing checks, and it cost a run on 2026-09-07."
    Write-Host "App Check is enforced on the AI callables; a debug build gets its"
    Write-Host "token from APP_CHECK_DEBUG_TOKEN at BUILD time, and empty is not a"
    Write-Host "request to generate one -- the backend rejects it, activate() throws,"
    Write-Host "and every call comes back 'unauthenticated', which says nothing about"
    Write-Host "App Check and reads like a sign-in fault while the log shows the user"
    Write-Host "signed in. Set `$env:APP_CHECK_DEBUG_TOKEN before building."
    Write-Host ""
